...
def retest(testloader, model, use_cuda, criterion, epoch, RESUME,camOutputDir):
    model.eval()
    test_loss = 0
    correct = 0
    ...
    # Save checkpoint.
    if epoch % 10 == 0:
        torch.save(model.state_dict(), camOutputDir+'checkpoint/' + str(RESUME + int(epoch / 10)) + '.pt')
